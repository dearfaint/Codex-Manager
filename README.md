...
## 功能概览
- 账号组管理：提供独立账号组页面，支持创建、编辑、停用与排序，平台 Key 可绑定账号组，避免账号池分组规则分散维护
- 账号池管理：分组、标签、排序、备注、封禁识别与封禁筛选，支持按订阅类型、账号组和状态筛选，并可对选中账号快捷更改账号组
- 批量导入 / 导出：支持多文件导入、桌面端文件夹递归导入 JSON、按账号导出单文件
- 用量展示：支持标准 5 小时 + 7 日窗口、仅 7 日单窗口账号，以及 Code Review / Spark 等官方附加额度窗口；刷新后会统一展示各额度的剩余百分比与重置时间，并按本地消耗金额与官方用量占比估算 5 小时 / 7 天剩余额度金额
- 授权登录：浏览器授权 + 手动回调解析
- 平台 Key：随机生成或自定义固定 Key、禁用、删除、模型绑定、账号组绑定、推理等级、服务等级（跟随请求 / Fast / Flex）
...
## 页面展示
### 桌面端
- 账号组管理：集中维护账号组名称、描述、状态与排序，并为平台 Key 和账号池分配提供统一入口
- 账号管理：集中导入、导出、刷新账号与用量，支持订阅类型 / 账号组 / 状态筛选、快捷更改账号组、低配额 / 封禁筛选、重置时间与剩余额度金额估算
- 平台 Key：按模型、账号组、推理等级、服务等级绑定平台 Key，并查看调用日志
